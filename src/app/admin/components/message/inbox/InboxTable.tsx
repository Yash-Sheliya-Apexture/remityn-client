...
import { format, formatDistanceToNow } from 'date-fns';
import { motion } from 'framer-motion';
import { Pencil, Trash2, Circle, CircleCheckBig } from 'lucide-react'; // Removed RefreshCw from imports
import { Skeleton } from '@/components/ui/skeleton';
import InboxTableHeader, { InboxSortField } from './InboxTableHeader';
import type { AdminInboxMessage } from '../../../../services/admin/inbox';
import { cn } from '@/lib/utils';
...
const InboxTable: React.FC<InboxTableProps> = ({
  ...
  const getStatusBadge = (isRead: boolean) => {
    const baseClasses = "inline-flex items-center justify-center px-4 py-1 rounded-full font-medium capitalize w-28";
    if (isRead) {
      return (
        <span className={cn(baseClasses, "text-gray-600 bg-gray-100 dark:bg-gray-600/20 dark:text-gray-400")}>
          <CircleCheckBig className="mr-1.5 size-3.5 flex-shrink-0" /> Read
        </span>
      );
    }
    return (
      <span className={cn(baseClasses, "text-blue-600 bg-blue-100 dark:bg-blue-600/20 dark:text-blue-400")}>
        <Circle className="mr-1.5 size-3.5 fill-current flex-shrink-0" /> Unread
      </span>
    );
  };
